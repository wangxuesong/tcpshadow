...
func TestQueryCIdescribeHandler2(t *testing.T) {
	_, pgBackend := net.Pipe()
	gbFront, gbBackend := net.Pipe()
	filter := NewQueryFilter()
	ctx := &Context{
	...
	//ctx.SetMetaData("QueryStage", "QueryIDescribeDone")
	ctx.SetMetaData("Bindcondition", 1)
	ctx.SetMetaData("Idnumber", int16(0))
	ctx.SetMetaData("Groupcount", 2)
	ctx.SetMetaData("Bindtype", []uint32{23})
	ctx.SetMetaData("Datebindint", uint16(1))
	ctx.SetMetaData("Datebindchar", "")
	filter.SetHandler(&cidescribeHandler{})
...
func TestQueryExecuteHandler2(t *testing.T) {
	_, pgBackend := net.Pipe()
	gbFront, gbBackend := net.Pipe()
	filter := NewQueryFilter()
	ctx := &Context{
